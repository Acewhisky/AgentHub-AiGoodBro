...
5. direct 路由由 `run` 托管实际进程和心跳；Hub 路由使用 `sync-hub` / `watch-hub` 绑定并读取真实 task ID。
...
经授权的维护记录使用 maintenance 路由，可以在 Next 中显示“在线·维护中”。先等现有 CLI 与 Hub 活跃任务结束，再逐账号占位；不创建假 Hub 任务、不发送模型请求。旧 Hub 未读取共享状态时，维护人员还需暂时关闭其既有接单开关并保存每项原值。
...
只有接入协议的入口才能共享预约原子性。旧 CLI、原生交互终端和绕过 Skill 的 Hub API 不会自动登记；注册表为空不能替代真实进程检查。一个 CODEX_HOME 也不等于独立的 macOS 桌面或浏览器权限。
