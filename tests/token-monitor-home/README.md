# Token home projection regression · 0915v1
...
The assertions cover unknown versus real zero, partial sources, unknown cost independent of complete Token coverage, Int64 bounds, stale cache timestamps, selected timezone, both reset types, multiple events on one day, all53 announcement ordering, real channel-result ordering and full JSON dimensions. Compared with 0913v1, the suite also checks today / seven calendar days / month-to-date totals across DST, the collector's top-level history and chart-compatible fallback, future buckets, duplicate dates, and overflow. They do not prove native dragging, fullscreen, live notification delivery or installed UI layout.
