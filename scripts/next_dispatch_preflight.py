...
REASON_LABELS = {
    ...
    "invalid_execution_preference": "保存的模型、推理强度或速度无效",
...
def execution_preference(profile: dict[str, Any]) -> dict[str, str] | None:
    preference = profile.get("executionPreference")
    if preference is None:
        return {"model": "gpt-6-astra", "reasoningEffort": "low", "serviceTier": "default"}
    ...
    if not all(isinstance(value, str) for value in (model, effort, tier)):
        return None
    if model not in maxima or effort not in ranks or ranks[effort] > maxima[model] or tier not in ("default", "fast") or (model == "gpt-5.2" and tier == "fast"):
        return None
    return {"model": model, "reasoningEffort": effort, "serviceTier": tier}
...
def self_test() -> None:
    ...
    assert policy_report(before_cutoff, astra_snapshot)["selected"]["executionPreference"] == astra_snapshot["profiles"][0]["executionPreference"]
