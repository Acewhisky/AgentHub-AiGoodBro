...
class Registry:
    ...
    def reserve(self, *, account_key: str, alias_key: str, code: str | None,
        ...
        if route not in {"direct", "hub", "warmup", "maintenance"}:
        ...
        with self.lock():
            ...
            old = [x for x in state["leases"] if x["state"] not in ACTIVE][-100:]
            state["leases"] = [x for x in state["leases"] if x["state"] in ACTIVE] + old + [lease]
            self._write(state)
            return lease
...
def supervise(registry: Registry, lease: dict, command: list[str], cwd: Path,
              *, env: dict | None = None, stdin=None, before_start=None) -> int:
    ...
    try:
        ...
        stop.set()
        thread.join(timeout=1)
        still_running = child is not None and (child.poll() is None or group_has_live_process(child.pid))
...
def parser():
    ...
    for key in ("lease-id", "owner", "code", "codex-bin", "brief-file", "output", "capability-report"):
...
def execute(args, registry):
    if args.command == "status":
        ...
        preference = {"model": args.model or preference["model"], "reasoningEffort": args.effort or preference["reasoningEffort"],
                      "serviceTier": args.service_tier or preference["serviceTier"]}
        if pre.execution_preference({"executionPreference": preference}) is None:
            raise ActivityError("execution_preference_invalid")
        executable = Path(args.codex_bin).expanduser().resolve()
        if executable.name != "codex" or not os.access(executable, os.X_OK):
        ...
        command = [str(executable), "exec", "--skip-git-repo-check", "-C", str(args.cwd.resolve()), "--sandbox", args.sandbox,
                   "--model", preference["model"], "-c", 'model_reasoning_effort="' + preference["reasoningEffort"] + '"',
                   "-c", 'service_tier="' + preference["serviceTier"] + '"',
                   "--enable" if preference["serviceTier"] == "fast" else "--disable", "fast_mode",
                   "--output-last-message", str(Path(args.output).expanduser().resolve()), "-"]

        def before_start():
            capability = json.loads(Path(args.capability_report).read_text())
            if capability.get("status") != "passed":
                raise ActivityError("capability_check_not_passed")
            if capability.get("cliSHA256") != hashlib.sha256(executable.read_bytes()).hexdigest():
        ...
        with Path(args.brief_file).open("rb") as brief:
