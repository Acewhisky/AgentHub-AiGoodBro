...
# The current product intentionally exposes only the Codex runtime. Keep the
# inherited Claude fixture ready for a future re-enable, but do not fail a
# release because its provider is outside the active product surface.
if ! grep -q 'ClaudeCodeRuntimeProvider' Sources/CodexUsageWidget/Providers/RuntimeProvider.swift; then
  echo "parser fixture skipped: Claude Code runtime is not enabled"
  exit 0
fi

if [[ "${CAMNEXT_SKIP_BUILD:-0}" != "1" ]]; then
  make build >/dev/null
fi
