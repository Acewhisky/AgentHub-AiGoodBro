...
def write_final(resources_dir):
    ...
    png_path = os.path.join(resources_dir, "AiGoodBro-icon.png")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        subprocess.run(
            ["iconutil", "-c", "icns", iconset, "-o", os.path.join(resources_dir, "AiGoodBro.icns")],
            check=True,
        )
...
def main():
    ...
    if not (args.candidates or args.final or args.verify_icons or args.board):
        parser.print_help()
        return 1
    if args.candidates:
