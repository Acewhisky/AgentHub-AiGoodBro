...
const ids = Object.fromEntries(['context','overview','trends','details','calendar','month','group','from','to','date','bars','legend','selection','dimensions','period'].map(id => [id,new Element()]));
ids.group.value = 'client'; ids.period.value = 'day';
const tabs = ['overview','trends','details'].map(mode => new Element({'data-mode':mode}));
const document = {documentElement:{},querySelector:()=>new Element(),getElementById:id => ids[id], querySelectorAll:selector => selector === '[data-mode]' ? tabs : [], createElement:() => new Element()};
const window = {};
...
assert(cell('2026-09-12').classes.has('known'));
assert(cell('2026-09-13').classes.has('partial'));
...
assert(cell('2026-09-12').classes.has('known'),'global/source/cost partial cannot corrupt known zero');
cell('2026-09-12').fire('click');assert.match(ids.selection.textContent,/0 Token · known/);assert.match(ids.selection.textContent,/Cost \(USD\): Not provided/);
...
assert.match(ids.selection.textContent,/Cost \(USD\): Not provided/,'original engine unknown zero must be unavailable');
assert.match(ids.selection.textContent,/130 Token · known/);
for (const language of ['zh','en']) {
  ...
  for (const coverage of [undefined,'unknown','known','partial']) for (const value of [0,2.5,null,undefined,'4',{},-1,Infinity,NaN]) {
    ...
    assert.match(ids.selection.textContent,expected);assert(cell('2026-09-13').classes.has('known'));
    cell('2026-09-13').fire('click');ids.period.value='day';ids.period.fire('change');assert.match(contents(ids.dimensions),expected);
  }
}
const missingCost=JSON.parse(JSON.stringify(engine));delete missingCost.coverage.cost;render(missingCost);cell('2026-09-13').fire('click');assert.match(ids.selection.textContent,/Cost \(USD\): Not provided/);
...
const scopedDay=JSON.parse(JSON.stringify(engine));scopedDay.coverage.days[0].cost='known';render(scopedDay);cell('2026-09-13').fire('click');assert.match(ids.selection.textContent,/Cost \(USD\): 0 · known/);
scopedDay.payload.history.daily[0].coverage={cost:'unknown'};render(scopedDay);assert.match(ids.selection.textContent,/Cost \(USD\): Not provided/);
ids.period.value='day';
const observed=JSON.parse(before); observed.coverage.days[0].status='unknown';render(observed);cell('2026-09-11').fire('click');assert.match(ids.selection.textContent,/90 Token.*observed value, completeness unconfirmed/);assert.match(ids.month.textContent,/partial\/unknown/);
for(const value of [null,undefined,'4',-1,Infinity,NaN]) { const x=JSON.parse(before);x.payload.aggregate.history.daily[1].tokens=value;render(x);assert(cell('2026-09-12').classes.has('unknown'));cell('2026-09-12').fire('click');assert.match(ids.selection.textContent,/Tokens unavailable/); }
const mapped=JSON.parse(before);mapped.coverage.days={'2026-09-12':{status:'known'}};render(mapped);assert(cell('2026-09-12').classes.has('known'));
const absent=JSON.parse(before);delete absent.coverage;render(absent);assert(cell('2026-09-12').classes.has('unknown'));
...
render(largeJSON);cell('2026-09-13').fire('click');assert.match(ids.selection.textContent,/130 Token · known/);
...
assert.match(ids.selection.textContent,/reset 7/);
