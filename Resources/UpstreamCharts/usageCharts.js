...
(function exposeUsageCharts(root, factory) {
  ...
  function areaLineSvg(model) {
    ...
    const point = model.points && model.points.length === 1
      ? `<circle class="area-line-point" cx="${svgRound(model.points[0].x)}" cy="${svgRound(model.points[0].y)}" r="3"></circle>`
      : '';
    return `<svg class="area-line" viewBox="0 0 ${model.width} ${model.height}" preserveAspectRatio="none" width="100%" height="100%" aria-hidden="true">${defs}${fill}${line}${point}</svg>`;
  }
