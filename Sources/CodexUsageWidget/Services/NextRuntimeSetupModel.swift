...
final class NextRuntimeSetupModel: ObservableObject {
    ...
    init(preview: Bool = false) {
        isPreview = preview
        if preview {
            report = Report(schemaVersion: 1, components: [
    ...
    func chooseExecutable(for id: String) {
        ...
        panel.message = language.text("选择已安装的 \(id == "python" ? "Python 3.9 或更新版本" : "Codex CLI")。选择后会验证版本与所需能力。", "Choose an installed \(id == "python" ? "Python 3.9 or newer" : "Codex CLI"). Its version and required capabilities will be checked.")
    ...
    private func execute(_ command: String, project: URL? = nil) {
        ...
        guard let resources = Bundle.main.resourceURL else { failed = true; return }
        ...
        DispatchQueue.global(qos: .utility).async { [weak self] in
            ...
            if let python = snapshot.python, let codex = snapshot.codex {
                ...
                do {
                    let data = try BoundedLocalProcess.run(executable: python, arguments: arguments, environment: NextRuntimeEnvironment.environment,
    ...
    func failureMessage(_ language: WidgetLanguage) -> String {
        ...
        case "codex_check_failed", "codex_capability_missing":
            return language.text("Codex CLI 可执行文件已变化或缺少调度能力。请升级或重新选择后再检查；不会改写原设置。", "The Codex CLI changed or lacks required dispatch capabilities. Update or choose it again; existing settings are unchanged.")
        ...
        case "account_snapshot_invalid", "account_identity_invalid":
            return language.text("账号快照或独立资料目录未通过身份校验。请在工作台重新登录相关账号，不要手动搬移资料目录。", "The account snapshot or isolated profile folder failed identity checks. Sign in again from the workspace; do not move profile folders manually.")
        ...
        case "existing_hub_preserved", "existing_hub_configuration_preserved":
            return language.text("发现已有 Hub 状态或配置，Next 未启动第二个服务。请从原部署入口恢复或核实现有服务后再检查。", "An existing Hub state or configuration was found, so Next did not start another service. Restore or verify the existing deployment, then check again.")
        case "hub_start_needs_review":
            return language.text("Hub 启动请求已发出，但健康状态尚未确认。不要重复启用；请先核实现有服务，再重新检查。", "The Hub start request was sent but health is not confirmed. Do not enable it again; verify the existing service, then check again.")
        ...
        case "directory_symlink_conflict", "directory_permission_conflict", "existing_file_conflict", "runtime_link_conflict", "setup_lock_invalid":
            return language.text("配套目录存在软链接、权限或文件冲突。原文件已保留；请核对 Next 私有目录后重新检查。", "A symlink, permission, or file conflict exists in the companion folders. Existing files were preserved; verify Next's private folders and check again.")
        case "rollback_conflict":
            return language.text("回滚时检测到文件被另一操作改动，Next 已保留较新的文件并停止。请核对配套目录后重新检查。", "A file changed during rollback. Next preserved the newer file and stopped. Verify the companion folder, then check again.")
        case "command_cleanup_failed":
            return language.text("依赖检查的子进程未能在限定时间内清理。请不要重复启用 Hub，先结束该次检查后再复检。", "A dependency-check subprocess could not be cleaned up in time. Do not enable another Hub; finish that check before retrying.")
        default:
            return language.text("配套工具准备未完成。请重新检查安装与目录权限；已有配置会保留。", "Companion setup did not finish. Recheck installation and folder permissions; existing configuration is preserved.")
        }
    }
}
