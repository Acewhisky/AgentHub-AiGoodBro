...
struct GlobalShortcut: Hashable {
    ...
    static let enabledStorageKey = "CodexManagerNext.globalShortcut.enabled"
    ...
    static func load(defaults: UserDefaults = .standard) -> GlobalShortcut? {
        ...
        guard modifiers != 0, modifiers & ~allowedModifiers == 0 else {
            return repairToDisabled(defaults: defaults)
        }
    ...
    private static func repairToDisabled(defaults: UserDefaults) -> GlobalShortcut? {
