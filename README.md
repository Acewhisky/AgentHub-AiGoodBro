...
当前源码版本：`0908v3` · `9.5.8 (22)`。项目非 OpenAI 官方产品，不提供账号、不增加额度，也不绕过登录、MFA 或平台限制。
...
5 小时与 7 天暖号分别开关。首次安装默认开启；升级保留原有开关，包括旧版本未手动保存的关闭状态。暖号会发送真实最小请求并消耗额度，可随时关闭。
...
8. 首次安装默认开启两项智能暖号，可按需关闭；低额度提醒与飞书仍需主动开启。
