...
当前源码版本：`0908v6` · `9.5.11 (25)`。已完成本机维护验证，尚未发布该版本安装包。项目非 OpenAI 官方产品，不提供账号、不增加额度，也不绕过登录、MFA 或平台限制。

0908v6 相比 0908v5：七项日常维护与通知功能默认开启，新账号继续默认参与调度；已保存的关闭选择保留，可在引导里“全部开启”。新增四步使用引导，支持跳过、续看和重新进入，明确区分功能开关、macOS 授权和飞书连接。详见 [0908v6 更新说明](docs/release-notes-v9.5.11.md)。

上一版 0908v5 相比 0908v4：明确显示维护参数暂停的功能，保护原有暖号偏好；允许分别设置 5 小时与 7 天低额度提醒线；增加默认关闭的 macOS 原生通知。继承 0908v4 的单实例锁、跨进程状态事务、运行中构建保护、稳定调度编号及额度失败拦截。当前运行验收与尚未完成项统一记录在 [0908v6 汇总说明](docs/release-notes-v9.5.11.md)。

0908v3 相比 0908v2：不参与调度的账号也照常刷新额度、会员日期，并按全局开关执行 5 小时和 7 天暖号。过期会员日期默认通过官方凭证刷新后重查，失败自动退避，并明确展示核查结果。保留上一版行为：5 小时与 7 天额度到期后默认刷新，即使暖号关闭、账号不参与调度或旧额度为 0 也会检查；失败或旧的耗尽窗口按间隔继续检查。首次安装默认开启暖号，升级保留已有开关，并修复登录失效的错误分类。本版已完成本机覆盖安装验证，源码与资料同步更新，详见 [0908v3 更新说明](docs/release-notes-v9.5.8.md)。
...
右上角截图按钮使用原生 SwiftUI/AppKit 渲染当前布局与展开状态下的完整工作台，列表和卡片均包含滚动区外的账号，不截其他窗口、临时弹窗或系统标题栏。通过系统保存面板写入本机 PNG，不会上传，也不需要录屏权限。默认窗口为 980 × 700；列表保持左侧身份、中间额度、右侧操作，卡片按窗口宽度自动排列为多列。长备注允许单行省略，完整资料可点“详情”；异常提示按需增高，不强行裁切。

排序仅在编辑模式显示手柄；松手到账号区域后保存一次，拖到区域外或按 Esc 会取消。右键手柄仍可上移、下移；系统开启“减少动态效果”时关闭排序过渡。切换布局不改变账号、监控目标、启动账号或调度范围。
...
- 单账号同样支持自动暖号。暖号开启后，Next 按账号窗口与空闲状态发送最小请求，尝试启动下一轮计时；5 小时和 7 天分别控制，不必每轮手动发消息。暖号会消耗额度，条件与时间规律见[智能暖号](#智能暖号)。
...
5 小时与 7 天暖号分别开关，未保存过的选项默认开启；升级保留明确保存的关闭选择。暖号会发送真实最小请求并消耗额度，可随时关闭。
...
单账号和多账号都能启用这项功能。当前实现要求 Next 持续运行、电脑保持唤醒并联网，且账号身份、额度、Hub 映射和空闲状态检查通过。5 小时和 7 天暖号分别跟随全局开关，不受“参与调度”影响；关闭参与调度后，两种暖号及额度、会员日期刷新照常。它不会唤醒关机或睡眠的电脑，不会增加额度，也不会调用官方重置券。工作时段限制和每日维护请求上限仍是讨论方向，尚未实现。
...
1. 先做纯额度刷新，确认身份与新鲜额度。
2. 用调度编号映射或现有 Hub 账号目录确认别名，再检查新鲜 Hub 概览中没有同别名活跃任务；缺少任务列表时暂停。
...
暖号期间会阻止添加账号，以及登录、删除、切换和并发手动刷新。启用暖号时全账号每 10 分钟纯刷新；全部关闭时仍每 30 分钟纯刷新。已知官方重置时间到达后另行刷新额度，不受暖号开关、参与调度或旧额度为 0 的影响；读取失败或到期后仍返回旧的耗尽窗口时，至少间隔 60 秒再次检查。读取失败保留旧快照；只有新鲜且未被后续失败覆盖的额度才允许进入暖号判断。7 天剩余额度不高于 5% 时会暂停 5 小时自动暖号。
...
> 此图为旧版界面；0908v6 起功能默认开启，可在使用引导或自动化中心调整。

该功能默认开启，是基于本机已保存数据的候选提示，不是自动派单或自动切号。当前推荐条件为：

- 源账号官方 5 小时剩余 `<= 5%`，或 7 天剩余 `< 10%`；这是默认提醒线，可在自动化中心分别选择 5%、10%、15%、20% 或 25%。
...
提醒阈值设置只改变提示时机；候选额度、身份与任务状态核验及一小时提醒间隔不变。维护启动参数暂停的功能会显示“维护期间暂停”，对应开关不能误改；原持久偏好保留，普通启动后恢复。

## macOS 系统通知

自动化中心的“低额度系统通知”默认开启；在使用引导或自动化中心点击“允许系统通知”才请求 macOS 授权。开关开启但尚未授权时显示“待系统授权”，不会自动弹权限窗口。低额度提醒开启并满足原有评估条件时，通知仅包含固定说明和剩余百分比，不包含账号资料。飞书仍可独立使用。

拒绝授权、系统关闭提醒样式或提交失败会明确显示原因。成功回执只表示通知已提交给 macOS，最终横幅显示仍取决于系统通知和专注模式设置。

## 飞书通知

飞书通知及两类额度事件默认开启。未配置机器人时显示“待配置”，保留开关选择；连接完成后才发送通知。
...
当前仓库未发布 `0908v6 / 9.5.11` GitHub Release 安装包，也未提供经过 Apple Developer ID 签名与公证的该版安装包。当前安装方式是从源码在目标 Mac 本机构建；不要把 Actions 产物当成已公证发行版。
...
8. 使用引导分为认识工作台、默认功能、连接通知、开始使用四步，可跳过并从“使用引导”入口续看。两项暖号、低额度提醒、系统通知、飞书通知、额度重置及 Reset 卡提醒默认开启；通知在完成系统授权或飞书配置后生效。

## 更新与卸载

更新源码前，先正常退出正在运行的 Next，并保留需要的恢复备份：
...
用新构建覆盖原安装路径后再打开，确认只运行一个 Next 实例。升级保留原有暖号开关，不会自动打开低额度提醒、飞书或 Desktop 切换。
